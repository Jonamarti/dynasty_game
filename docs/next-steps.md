...
| **M11 phase 10, continued, and phase 11** | **next.** `bread`, `brewing`, `herding`, `dairying`, `wool`, `wattle_daub`, `masonry`, `kiln`, `well`, `calendar`, `the_wheel` — several need a real mechanism rather than a numeric term, and the Neolithic era rung still waits on `herding` and `masonry` — then war. See [m8_plan_the_ages.md](m8_plan_the_ages.md) |
