...
| **M11 phase 10, continued, and phase 11** | **next.** `dairying`, `wool`, `brewing`, `well`, `kiln` — `wool`/`dairying` depend on `herding` (now shipped), `well`/`kiln` depend on `masonry` (now shipped) — then war. See [m8_plan_the_ages.md](m8_plan_the_ages.md) |
