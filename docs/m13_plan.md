# M13 — El mundo más allá de la comarca

Escrito el 2026-09-24, al cerrar M12 y procesar `docs/notes3.txt`. Reúne tres
cosas que hasta hoy estaban dispersas:

1. **El mapa del mundo**, aplazado dos veces: de M12 en `m11_plan.md` («Para el
   futuro»), y a M13 en `m12_plan.md` («Después: M13»). Casillas, icono de
   globo, migración, caravanas, civilización.
...
### 0b. Las referencias a M13, verificadas

- `m12_plan.md`, «Después: M13»: casillas, globo, migración, civilización como
...
- `Justice.ts` se declara «la semilla de la ley escrita que M13 pide».
...
| `m9_6_plan.md` fases 4b-4d | Los canales de ánimo `comfort`, `belonging` y `purpose` sin quien los escriba ni los lea | **Fase 4** (4b los escribe al dormir; 4d los lee) |
| `m9_6_plan.md` fase 5 | Dormir al raso: hoy `sleep` exige un edificio | **Fase 4a**, porque la concepción lee dónde durmió cada uno |
...
| `m8_plan_the_ages.md` M8.4 | Hierro | **Después de M13** (M14); el hierro de pantano espera comarcas de humedal, que la fase 11 crea |
...
| `m11_plan.md` y la memoria del propietario | El archienemigo multigeneracional | **Fase 18e**; M12 fase 6 ya hereda las enemistades y M13 sólo tiene que conservarlas entre comarcas |
...
Lo que **no** entra en M13, a propósito: la hierro-metalurgia (M8.4), la
...
[m13_phase1.md](m13_phase1.md). La matriz roja y la pregunta de `bands-take-sides`
...
- **4a. Dormir al raso existe.** `doSleep` exige un edificio y quien no tiene
...
- **4b. `comfort` y `security` al dormir.** El suelo desnudo cuesta `comfort`,
...
- **4d. Leer el ánimo** (M9.6 4c). La tristeza empuja hacia compañía,
...
- **6e. El destete y la cuna.** Pasado el año el niño anda, despacio, y se
  queda cerca de la madre o del campamento. **Tecnología nueva `cradle`**
...
- **8a. La hoguera.** Edificio `hearth` bajo `firemaking`, barato (palos y
...
- **8b. Carne asada.** Recetas `roast_meat` y `roast_fish` en la hoguera, bajo
...
- **13d. La deuda de interfaz.** Esta es la pasada de interfaz del milestone.
  ...
  (M13 añade más de diez), y el zoom de `FamilyTree` en el móvil
...
- **18e. La casa rival.** Las enemistades de hogar (M12 6a-6c) viajan en los
  ...
  propietario pidió no construir antes de tiempo; M13 sólo evita que se pierda
...
convertirse en M14** si el bloque VII se alarga. Está escrito para que no haya
...
| 4a dormir al raso · 4b términos · 4c concepción bajo techo · 4d ánimo | uno a uno, medidos |
...
| 8a hoguera · 8b asado · 8c crudo | medidos |
...
8. **¿El bloque VIII es M13 o M14?** *Recomendado:* M14, con este documento
   como su plan.
